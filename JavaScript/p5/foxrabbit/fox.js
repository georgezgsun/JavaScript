function Fox() {
	...
	this.grow = function () {
		...
		if (age >= deadAge || energy <= 0) {
			space[4] = 0;	// die of energy or aged
		} else {
			space[4] = energy * 100 + age;  // update the age and energy index
	...
	this.hunt = function() {
		if (this.hasRoom(1)) {
			...
			space[this.searchRoom(1)] = energy*100 + age + 1000; // the same fox move to new location;
			space[4] = 0; // the previouse location has nothing now
		} else {
			if (this.hasRoom(0)) {
				space[this.searchRoom(0)] = space[4] + 1000;   // fox jump to an empty cell;
			}
